...
impl ClassReconstructor {
    ...
    pub fn generate_cpp_output(
        ...
        if config.detect_namespaces {
            ...
            let mut sorted_namespaces: Vec<String> = namespace_classes.keys().cloned().collect();
            sorted_namespaces.sort();
            
            for namespace in &sorted_namespaces {
                let mut namespace_class_list = namespace_classes[namespace].clone();
                namespace_class_list.sort_by(|a, b| a.name.cmp(&b.name));
